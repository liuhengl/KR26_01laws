...
def load_and_symbolize_tpch_data_q4(instance_folder):
    print(f"Loading TPC-H data from {instance_folder}...")
    try:
        schemas = {
            'lineitem': ['l_partkey', 'l_suppkey'], 'orders': [],
            'supplier': ['s_suppkey', 's_nationkey'], 'part': [], 'nation': []
        }
        dfs = {tbl: pd.read_csv(os.path.join(instance_folder, f'{tbl}.csv'), delimiter=',', header=0, dtype=str)
               for tbl in ['orders', 'lineitem', 'part', 'supplier', 'nation']}

        nations_df = dfs['nation']
        nation_key = nations_df[nations_df['n_name'] == 'CANADA']['n_nationkey'].iloc[0]

        parts_df = dfs['part']
        purple_part_keys = set(parts_df[parts_df['p_name'].str.contains('purple', na=False)]['p_partkey'])

        suppliers_z3 = {
            row['s_suppkey']: to_z3_representation(row, schemas['supplier'], 's_suppkey')
            for _, row in dfs['supplier'].iterrows()
        }

        us_suppliers_z3 = {
            s_key: s_z3 for s_key, s_z3 in suppliers_z3.items()
            if simplify(s_z3['s_nationkey'] == StringVal(nation_key)) == True
        }

        lineitems_z3_by_okey = defaultdict(list)
        for _, row in dfs['lineitem'].iterrows():
            okey = int(row['l_orderkey'])
            lineitems_z3_by_okey[okey].append(to_z3_representation(row, schemas['lineitem'], 'l_orderkey'))

        print("TPC-H data loaded and pre-processed.")
        return lineitems_z3_by_okey, purple_part_keys, us_suppliers_z3
    except Exception as e:
        print(f"An error occurred during data loading: {e}")
        return None

def generate_phi_for_query_4(targeted_okeys, lineitems_z3_by_okey, purple_part_keys, us_suppliers_z3):
    phi_map = {}
    print(f"Generating phi for {len(targeted_okeys)} targeted order keys...")
    us_supplier_keys_z3 = [s['s_suppkey'] for s in us_suppliers_z3.values()]

    for o_key in targeted_okeys:
        subquery_disjuncts = []
        for l_z3 in lineitems_z3_by_okey.get(o_key, []):
            part_cond = Or([l_z3['l_partkey'] == StringVal(pk) for pk in purple_part_keys])
            supp_cond = Or([l_z3['l_suppkey'] == sk_z3 for sk_z3 in us_supplier_keys_z3])
            lineitem_cond = And(part_cond, supp_cond)
            subquery_disjuncts.append(lineitem_cond)

        exists_formula = Or(subquery_disjuncts) if subquery_disjuncts else BoolVal(False)
        phi_map[o_key] = simplify(Not(exists_formula))
...
def parse_arguments():
    ...
    parser.add_argument("--cache_path", type=str, default="phi_Q4_cache.json")
    parser.add_argument("--keys_file", type=str, default="targeted_orderkeysQ4.json")
...
if __name__ == '__main__':
    ...
    k_configs_to_test = [{'mode': 'relative', 'range': range(1, 51)}]
    ...
    default_targeted_orderkeys = [1, 2, 3, 4, 5, 6, 7, 32, 33, 34, 35, 36, 38, 39, 64, 65, 66, 67, 68, 69, 70]
    ...
    if phi_map is None:
        print("No cache found. Generating new phi_map for Query 4...")
        processed_data = load_and_symbolize_tpch_data_q4(DATA_PATH)
        if processed_data:
            lineitems_z3, yellow_parts, us_suppliers = processed_data
            phi_map = generate_phi_for_query_4(targeted_orderkeys, lineitems_z3, yellow_parts, us_suppliers)
        ...
        for k_config in k_configs_to_test:
            for dist_config in dist_configs_to_test:
                for shuffle in shuffle_options:
                    ...
                    output_filename = f"results/Query4_Analysis_{key_to_analyze}_{dist_config['name']}_{k_config['mode']}_{shuffle_str}.csv"
